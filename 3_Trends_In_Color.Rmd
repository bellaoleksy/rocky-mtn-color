...
dwl_all %>%
  ...
  inner_join(lakezones %>% select(lagoslakeid, lake_centroidstate), by="lagoslakeid") %>%
  distinct(Hylak_id,.keep_all = T) %>%
  group_by(Trend, lake_centroidstate, ftype) %>%
  summarize(n = length(unique(Hylak_id))) %>%
  mutate(perc = (n / sum(n)) * 100,
         perc = round(perc, 0)) %>%
...
trends_df<-dwl_all %>%
  ...
  # filter(Trend!="No trend") %>%
  inner_join(lake_descriptor %>% select(lagoslakeid, Hylak_id), by="Hylak_id") %>%
  inner_join(lakezones %>% select(lagoslakeid, lake_centroidstate, epanutr_zoneid, neon_zoneid), by="lagoslakeid") %>%
  # filter(Trend %in% c("Blue -> Green",
  #                     "Green -> Blue")) %>%
  select(Hylak_id, Trend,epanutr_zoneid,neon_zoneid,lake_centroidstate) %>%
  ...
  select(-c(dWL,group,lake_connectivity)) %>%
  ...
  select(Hylak_id, Trend, res, log10ws_area, log10WALA,  elev, meandepth, slope, log10LSA, log10pop_sum, temp_trend_summer,
         ppt_trend_winter, temp_trend_summer, n_dep, slope_winter_ppt_mm, slope_summer_tmean_degC, lake_centroidstate,
         perc_urban, perc_forest) #
...
#Add weights, e.g, give higher weight to smaller class
...
cart_mod <- rpart(Trend ~ ., data = train_d,
                  method = 'class',
                  weights = modelWeights,
                  cp = 0.05) # CP is a tuning knob for tree complexity.
...
hist(trends_df$log10pop_sum, breaks=50)
hist(trends_df$slope_spring_tmean_degC, breaks=50)
hist(trends_df$elev, breaks=50)
...
temporal_conf_mat<-ggplot(data = plotTable,
  ...
  # geom_tile(aes(fill=Pos_Switch.to.Blue), color="black") +
    geom_tile(aes(fill=Prediction), color="black") +
...
temporal_tree_plot +temporal_conf_mat + plot_layout(widths = c(0.7,0.3))
#Combine
library(patchwork)
...
trends_df<-dwl_all %>%
  ...
  # mutate(logLSA=log(LSA),
  #        logWALA=log(WALA),
  #        logws_area=log(ws_area),
  #        logpop_sum=log(pop_sum)) %>%
  # select(-LSA, -WALA, -ws_area, -pop_sum)
...
#Code from: https://bgreenwell.github.io/pdp/articles/pdp.html
# fit_rf %>%  # the %>% operator is read as "and then"
#   partial(pred.var = "elev") %>%
#   autoplot(smooth = TRUE, ylab = expression(f(elev))) +
#   theme_few() 
# partial(fit_rf, pred.var = "elev", plot = TRUE, rug = TRUE, 
#         plot.engine = "ggplot2")

# fit_rf %>%  # the %>% operator is read as "and then"
#   partial(pred.var = "slope_spring_tmean_degC") %>%
#   autoplot(smooth = TRUE, ylab = expression(f(slope_spring_tmean_degC))) +
#   theme_few() 
# partial(fit_rf, pred.var = "slope_spring_tmean_degC", plot = TRUE, rug = TRUE, 
#         plot.engine = "ggplot2")
# 
# #Get fancy
# # Compute partial dependence data for lstat and rm
# pd <- partial(fit_rf, pred.var = c("elev", "slope_spring_tmean_degC"))
# 
# # Default PDP
# pdp1 <- plotPartial(pd)
# 
...
# 
# # Compute partial dependence data for lstat and rm
# pd <- partial(fit_rf, pred.var = c("elev", "cv_summer_tmean_degC"))
# 
# # Default PDP
# pdp1 <- plotPartial(pd)
# partial(fit_rf, pred.var = c("elev", "cv_summer_tmean_degC"), plot = TRUE, chull = TRUE,
#               palette = "magma", plot.engine="ggplot2")
# 
# #Where are all the positive slope lakes?
# pd <- partial(fit_rf, pred.var = c("slope_summer_tmean_degC", "slope_winter_tmean_degC"))
# 
# # Default PDP
# pdp1 <- plotPartial(pd)
# partial(fit_rf, pred.var = c("slope_summer_tmean_degC", "slope_winter_tmean_degC"), plot = TRUE, chull = TRUE,
#               palette = "magma", plot.engine="ggplot2")


#Another tidy way via: https://rviews.rstudio.com/2019/06/19/a-gentle-intro-to-tidymodels/

library(tidymodels)
tidymodels::trends_split <- initial_split(trends_df, prop = 0.6)
tidymodels::trends_split
...
```{r}
mn_res <- nnet::multinom(Trend ~ perc_forest + elev + perc_urban + slope + pop_sum, data = train_d, Hessian=T, ref="No trend")
summary(mn_res)
mn_pred <- data.frame(predict(mn_res, test_d, type = 'prob'))
# mn_pred <- data.frame(mn_pred)
colnames(mn_pred) <- c("NT","BB","BG","GG","BG")
