...
##IAO-- this is cool but I think we will want to only look at median. Or if we want to look at max, maybe filter down to only sites with >3 observations a year?
...
ggplot(dwl_all, aes(x = year, y = value, group = Hylak_id)) + 
  geom_smooth(method = 'lm',
              ...
              col = 'gray',
              alpha = 0.1) + 
  geom_point(data = full_spec, shape=21, fill = 'white') + 
  geom_smooth(data = full_spec, color = 'black',
              method = 'lm',
              se = FALSE) + 
  facet_wrap(~Trend) + 
  theme_few()
## IAO -- Add sample size to plots 
# dev.off()
```
