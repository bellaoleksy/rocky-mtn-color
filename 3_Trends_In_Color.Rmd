...
if(!require(lfstat)){install.packages("lfstat")};library(lfstat) #for water_year function
...
if(!require(prism)){install.packages("prism")};library(prism) 
...
co_summary <- ls_co %>%
  ...
  arrange(year) %>%
  mutate(impute_value = imputeTS::na_interpolation(value, maxgap = 1))
...
#### ~ For review
What does the distribution of number of samples per summer look like?
```{r, eval=F}

distrib_samples <- ls_co %>%
  dplyr::select(-year, -sat) %>%
  mutate(month = month(date),
         year = year(date),
         doy = yday(date)) %>%
...
#What does the distribution of number of samples look like overall?
distrib_samples %>%
  ggplot()+
...
#Export
png(filename = 'figs/Figure3.Trend Examples_updated.png',
    width = 5,
    height = 8,
    res = 600,
    units = 'in')
...
png(filename = 'figs/Figure4A.trend_map.png',
