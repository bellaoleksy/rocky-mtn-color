...
cart_mod <- rpart(Trend_simple ~ ., data = train_d,
                  method = 'class',
                  cp = 0.054) # CP is a tuning knob for tree complexity.
...
temporal_tree_plot<-ggparty(cart_mod_party) +
  ...
  geom_edge_label() +
  geom_node_splitvar() +
  geom_node_plot(gglist = list(geom_bar(aes(x = "", fill = Trend_simple),
...
#### Sample code
```{r}
#Code from https://github.com/limnoliver/CSI-Nutrient-Time-Series/blob/master/Code/06_analysis_rf.R
...
trends_df<-dwl_all %>%
  ...
  select(Hylak_id, Trend_simple, res, WALA, slope, LSA, slope_summer_tmean_degC, perc_barren,
