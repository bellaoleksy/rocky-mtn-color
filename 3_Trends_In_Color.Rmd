...
lakezones_studyregion_sf = st_as_sf(lakezones_studyregion, coords = c("lake_lon_decdeg", "lake_lat_decdeg"), 
                 crs = 4326, agr = "constant")


buffer_box <- st_bbox(lakezones_studyregion_sf) %>%
  st_as_sfc(.) %>%
  st_buffer(50000) %>%
  st_bbox()
...
# remotes::install_github("wmgeolab/rgeoboundaries")
library(rgeoboundaries)
elevation_data <- get_elev_raster(locations = states, z = 5, clip = "locations")
elevation_data <- as.data.frame(elevation_data, xy = TRUE)
colnames(elevation_data)[3] <- "elevation"
# remove rows of data frame with one or more NA's,using complete.cases
elevation_data <- elevation_data[complete.cases(elevation_data), ]
...
png(filename = 'figs/lake_size_map_WUS_all.png',
    width = 8, height = 9, units = 'in', res = 600)
ggplot() +
    geom_polygon(
    data = map_data("state"),
...
###Figure S1. DWL trend map (categorical)
```{r}
