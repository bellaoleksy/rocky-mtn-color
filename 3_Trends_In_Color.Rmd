...
trend_plotter <- function(study_lakes = high_lakes,
  ...
  #Second approach, same as original but add Mode_change category. 
    sens_sum = unnest(co_mods, c(sens_sum, slope, early_mean)) %>%
        left_join(.,site_cols %>%
               select(Hylak_id,group)) %>%
    mutate(
      Trend = case_when(
        p.value <= 0.05 & slope >= 0 & early_mean < 530 ~ 'Blue -> Greener',
        p.value <= 0.05 & slope >= 0 & early_mean >= 530 ~ 'Intensifying Green/Yellow',
    ...
    mutate(
      Mode_change = case_when(Trend == "Blue -> Greener" & group == "Green/murky" ~ "Change to Green/murky",
                              Trend == "Green -> Bluer" & group == "Blue/clear" ~ "Change to Blue/clear",
                              TRUE ~ "No net change")
    )
  
  
  # #First approach I tried to interrogate the trends further, 2022-03-24  
  #   sens_sum = unnest(co_mods, c(sens_sum, slope, early_mean)) %>%
  #       inner_join(site_cols %>%
  #              select(Hylak_id,group)) %>%
  #   mutate(
  #     Trend = case_when(
  #       p.value <= 0.05 & slope >= 0 & early_mean < 530 & group == "Green/murky" ~ 'Blue -> Green',
  #       p.value <= 0.05 & slope >= 0 & early_mean < 530 & group == "Blue/clear" ~ 'Blue -> Slightly Greener',
  #       p.value <= 0.05 & slope >= 0 & early_mean >= 530~ 'Intensifying Green/Yellow',
  #       p.value <= 0.05 & slope <= 0 & early_mean > 530 & group == "Blue/clear" ~ 'Green -> Blue',
  #       p.value <= 0.05 & slope <= 0 & early_mean > 530 & group == "Green/murky" ~ 'Green -> Slightly Bluer',
  #       p.value <= 0.05 & slope <= 0 & early_mean < 530 ~ 'Intensifying Blue',
  #       p.value > 0.05 ~ 'No trend'
  #     ),
  #     Trend = factor(
  ...
  #         'Intensifying Blue',
  #         'Green -> Blue',
  #         'Green -> Slightly Bluer',
  #         'Intensifying Green/Yellow',
  #         'Blue -> Green',
  #         'Blue -> Slightly Greener'
  #       )
  #     )
  #   )
...
I think we can delete the chunk below-- artifact of old analysis
...
dwl_all %>%
  ...
  mutate(perc = (n / sum(n)) * 100,
         perc = round(perc, 0)) %>%
  filter(Trend %in% c("Blue -> Greener","Intensifying Green/Yellow"))
...
dwl %>%
  ...
  ungroup()%>%
  select(Hylak_id, lake_namegnis, geometry, lake_centroidstate, lake_connectivity, res)
...
dwl %>%
  ...
  dplyr::select(Hylak_id,Trend, data,intercept,slope, Mode_change) %>%
  unnest(c(data,intercept)) %>%
  filter(Trend=="Green -> Bluer" & Mode_change == "Change to Blue/clear") %>%
  distinct(Hylak_id,.keep_all = T) %>%
  inner_join(lake_descriptor %>% select(lagoslakeid, Hylak_id, lake_namegnis), by="Hylak_id") %>%
  inner_join(lakezones %>% select(lagoslakeid, lake_centroidstate), by="lagoslakeid") %>%
  inner_join(blue_green_2018, by="Hylak_id") %>%
...
#For lakes that are switching from blue->green, how many of them were classified as green/murky in the spatial analysis?
dwl_all %>%
  filter(Trend=="Blue -> Green") %>%
  ggplot(aes(x=year,y=value, group=Hylak_id))+
  geom_point(shape=21)

dwl %>%
  filter(stat == 'median') %>%
  dplyr::select(Hylak_id,Trend, data) %>%
  unnest(data) %>%
  inner_join(site_cols,.) %>%
  filter(Trend %in% c("Blue -> Green",
                      "Blue -> Slightly Greener"))  %>%
  ggplot(aes(x=year,y=value, group=Hylak_id, fill=group))+
  geom_hline(yintercept=530, linetype="dashed")+
  geom_point(shape=21)+
  geom_smooth(aes(color=group),method="lm",se=F)+
  scale_fill_manual(values=c('#1f78b4','#33a02c'),
                    name="Spatial categorization")+
  scale_color_manual(values=c('#1f78b4','#33a02c'),
                    name="Spatial categorization")+
  theme_few()+
  labs(title="Temporal trends - lakes categorized as switch from blue -> green")+
  facet_wrap(~Trend)

#FAHHHCK. 

# png(filename = 'figs/trend_cats_expandedcategories.png',
...
dwl %>%
  ...
  ggplot(aes(x=year,y=value, group=Hylak_id, fill=group, shape=Mode_change))+
  geom_hline(yintercept=530, linetype="dashed")+
  geom_point()+
  geom_abline(aes(intercept = intercept, slope = slope, color=group),
              size=0.5, alpha=0.8) + #Sen's slope for selected sites
  scale_fill_manual(values=c('#1f78b4','#33a02c'),
                    name="Spatial analysis categorization")+
  scale_color_manual(values=c('#1f78b4','#33a02c'),
                    name="Spatial analysis categorization")+
  scale_shape_manual(values=c(21,22,25))+
  theme_few()+
  labs(title="Temporal trends - expanded categories")+
  facet_wrap(~Trend)+
...
## Look JUST a lakes that are going from blue->green
dwl_map_BG <- dwl %>%
  filter(stat == 'median') %>%
  dplyr::select(Hylak_id,Trend) %>%
  inner_join(site_cols,.) %>%
...
#     Variable = replace(Variable, Variable %in% c("slope_winter_ppt_mm"), "Change in winter\nprecip. (mm/year)"),
#     Variable = replace(Variable, Variable %in% c("pop_sum"), "Human pop."),
#     Variable = replace(Variable, Variable %in% c("LSA"), "Lake surface area")
#   ) %>%
#   ggplot(aes(
#     x = Importance,
#     y = forcats::fct_reorder(Variable, Importance, .desc = TRUE)
#   )) +
...
D <- ggplot(trends_df, aes(x = Trend_new, y = slope_winter_ppt_mm, fill = Trend_new)) +
  ...
  labs(y="Slope of winter precipitation\n1984-2020 (mm/year)")+
...
#### Alt version with Trend colors
```{r}


png(
  filename = 'figs/RF_results_alt.png',
  width = 4,
  height = 12,
  res = 600,
  units = 'in'
...
med_noTrend <-
  trends_df %>% filter(Trend_new == "No trend") %>% summarize(median(pop_sum+0.1)) %>%
  pull()
B<-  ggplot(trends_df, aes(x = Trend_new, y = pop_sum+0.1, fill = Trend)) +
  # geom_violin()+
...
med_noTrend <-
  ...
  pull()
C <-
  ggplot(trends_df, aes(x = Trend_new, y = elev, fill = Trend)) +
  # geom_violin()+
  geom_boxplot(fill=NA, outlier.shape = NA) +
  geom_jitter(
    aes(fill = Trend),
...
med_noTrend <-
  ...
  summarize(median(slope_winter_ppt_mm)) %>%
  pull()
D <- ggplot(trends_df, aes(x = Trend_new, y = slope_winter_ppt_mm, fill = Trend)) +
  geom_boxplot(fill=NA, outlier.shape = NA) +
  geom_jitter(
    aes(fill = Trend),
    shape = 21,
...
med_noTrend <-
  trends_df %>% filter(Trend_new == "No trend") %>% summarize(median(perc_urban+0.1)) %>%
  pull()
E <-ggplot(trends_df, aes(x = Trend_new, y = perc_urban+0.1, fill = Trend)) +
  # geom_violin()+
  geom_boxplot(fill=NA, outlier.shape = NA) +
  geom_jitter(
    aes(fill = Trend),
...
med_noTrend <-
  trends_df %>% filter(Trend_new == "No trend") %>% summarize(median(slope_spring_tmean_degC)) %>%
  pull()
eff <- ggplot(trends_df, aes(x = Trend_new, y = slope_spring_tmean_degC, fill = Trend)) +
  geom_boxplot(fill=NA, outlier.shape = NA) +
  geom_jitter(
    aes(fill = Trend),
    shape = 21,
