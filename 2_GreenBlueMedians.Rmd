...
author: "Bella Oleksy, Matthew Ross"
...
if(!require(ggparty)){install.packages("ggparty")};library(ggparty)
...
##Legend only
png(filename = 'figs/Figure1A.F-U_legend.png',
    width = 2, height = 11.8, units = 'in', res = 600)
...
#         plot.title=element_text(size=20, hjust=0.5, color="white", face="bold"),
#         
...
png(filename = 'figs/Figure1.spatial_color_map_continuous.png',
...
### Figure S1. Blue Green Category Only Map

```{r}



png(filename = 'figs/FigureS1.bg_map_categorical.png',
...
## Also note that lines 384-388 won't run with the modified header names on train_d, just FYI. 
...
cart_mod <- rpart(group ~ ., data = train_d,
                  ...
                  cp = 0.023) # CP is a tuning knob for tree complexity.
...
## Plot regression tree reults using ggparty
...
tree_plot<-ggparty(cart_mod_party) +
  ...
  geom_node_plot(gglist = list(geom_bar(aes(x = "", fill = group),
                               ...
                               (scale_x_discrete(expand = c(0, 0))), #removes white space
                               ylab("Proportion")),
...
## Create manually because the plot_confusion_matrix function doesn't have the customizations I want
conf_mat <- cvms::confusion_matrix(targets = test_d$group,
...
conf_matrix_plot<-ggplot(data = plotTable,
       mapping = aes(x = Target, y = Prediction, fill = Pos_Blue)) +
  geom_tile(aes(fill=Pos_Blue), color="black") +
...
#Combine & export
png(filename = 'figs/Figure2.spatial_plots.png',
...
## Figure SX. Correlations
Reviewer comment: "Although many indicators are involved in lake color, most of them are self-correlated. For example, high-altitude lakes are deeper, have steep-slope, and lower MAAT. Due to its low level of urbanization, it has a small population and a low proportion of cultivated land; and because it is located upstream, its pollutant enrichment level is low. Although this study has confirmed the original color or trends in lakes are associated with the above indicators, all associations can be attributed to the altitude. Obviously, if the purpose of this study is to prove the difference between the color of lakes at high and low altitudes, or to prove the relationship between altitude and MAAT, lake slope, depth or human population, etc., it does not make much sense."

Here I want to show the reviewer that the predictors are not nearly as collinear as you might expect. 

```{r}
## Get a correlation matrix
corr_df<-blue_green_2018 %>% select(-Hylak_id, -group, -res, -lake_connectivity, -dWL) %>% drop_na()
...
spatial_correlations<-flattenCorrMatrix(res$r, res$P)

#How many are significant at the p < 0.01 level?
spatial_correlations %>%
  filter(p<0.01) %>% ## uhhh basically all of them
  filter(row %in% c("air_temp","maxdepth","slope")) %>%
  arrange(desc(cor)) #this looks like a managable amount to plot. 


#Make bar plot
png(filename = 'figs/FigureSX.spatial_predictor_correlations.png',
...
spatial_correlations %>%
  ...
  filter(row %in% c("air_temp","maxdepth","slope")) %>%
  mutate(row =
          ifelse(row=="air_temp","MAAT (°C)",
          ifelse(row=="maxdepth", "max. depth",
          ifelse(row=="slope", "watershed slope"," ")))) %>%
  mutate(column=
       ifelse(column=="precip", "MM PPT (mm)", 
       ifelse(column=="air_temp","MAAT (°C)",
       ifelse(column=="LSA", "lake area",
       ifelse(column=="elev", "elevation",
       ifelse(column=="pop_sum", "human pop.",
       ifelse(column=="maxdepth", "max. depth",
       ifelse(column=="meandepth", "mean lake depth (m)",
       ifelse(column=="ws_area", "watershed area",
       ifelse(column=="no3_dep", "NO3 dep.",
       ifelse(column=="nh3_dep", "NH3 dep.",
       ifelse(column=="n_dep", "total N dep.",
       ifelse(column=="perc_ice", "% ice/snow",
       ifelse(column=="perc_urban", "% urban",
       ifelse(column=="sil", "% siliceous bedrock",
       # ifelse(column=="perc_decid", "% of catchment area classified as deciduous forest land cover",
       # ifelse(column=="perc_conifer", "% of catchment area classified as evergreen forest land cover ",
       # ifelse(column=="perc_mixedForest", "% of catchment area classified as mixed deciduous/evergreen forest land cover",
       ifelse(column=="perc_shrub", "% shrub/scrub",
       ifelse(column=="perc_grass", "% grassland/herbaceous",
       ifelse(column=="perc_wetland", "% wetland",
       ifelse(column=="perc_hay", "% hay",
       ifelse(column=="perc_crop", "% crop",
       ifelse(column=="perc_ag", "% ag.",
       ifelse(column=="perc_forest", "% forset",
       ifelse(column=="perc_barren", "% barren",
       ifelse(column=="slope", "watershed slope",
       ifelse(column=="WALA", "watershed area:lake area",
       ifelse(column=="cti", "composite topographic index"," ")))))))))))))))))))))))))) %>%
  arrange(desc(cor)) %>%
  mutate(name=paste(row, "-", column)) %>%
  ggplot(aes(x=cor,y=fct_reorder(name, cor, .desc = TRUE), fill=cor))+
  geom_bar(stat="identity", width=0.75, color="black")+
  scale_fill_gradient2(low = "red", mid="white", high = "darkgreen", na.value = NA) +
  geom_text(aes(label=round(cor,1)),color="black",size=3,position=position_stack(vjust=0.5)) +
  labs(y="Pairs",
...
Plot some of these relationship to help with writing the discussion.
Don't need to publish this section of the code, obviously. 
```{r}
...
covariates<-blue_green_2018 %>%
  ...
  filter(!skim_variable %in% list("Hylak_id","dWL","LSA","n_dep")) %>%
    ...
    mutate(Description=
           ...
           ifelse(skim_variable=="perc_ag", "% of watershed area classified as crop + hay",
           ifelse(skim_variable=="perc_forest", "% of watershed area classified as deciduous, confider, and mixed forset",
